    ...
    private void registerRoutes(Javalin javalin) {
        ...
        javalin.post("/api/auth/login", ctx -> {
            ...
            ctx.json(new AuthResponse(token,
                    UserMapper.toResponse(user, buildings, activeBuildingId, gcsStorageService)));
        });
        ...
        javalin.get("/api/finance/payments/{paymentId}/receipt", ctx -> {
            ...
            byte[] pdf = paymentReceiptPdfService.buildPaymentReceipt(payment, buildingName, unitLabel,
                    chargeDescription);
        ...
        javalin.get("/api/chat/neighbors", ctx -> {
            ...
            var resolved = neighbors.stream().map(n -> new com.domu.database.UserRepository.ChatNeighborSummary(
                    ...
                    UserMapper.resolveUrl(n.privacyAvatarBoxId(), gcsStorageService))).toList();
            ctx.json(resolved);
        });
        ...
        javalin.get("/api/admin/staff", ctx -> {
            ...
            if (user == null || user.roleId() == null || user.roleId() != 1L) {
                ctx.status(HttpStatus.FORBIDDEN);
                ctx.json(ErrorResponse.of("Solo administradores pueden ver el personal",
                        HttpStatus.FORBIDDEN.getCode()));
                return;
            }
            Long selectedBuildingId = validateSelectedBuilding(ctx, user);
        ...
        javalin.post("/api/admin/staff", ctx -> {
            ...
            if (user == null || user.roleId() == null || user.roleId() != 1L) {
                ctx.status(HttpStatus.FORBIDDEN);
                ctx.json(
                        ErrorResponse.of("Solo administradores pueden crear personal", HttpStatus.FORBIDDEN.getCode()));
                return;
            }
            Long selectedBuildingId = validateSelectedBuilding(ctx, user);
            if (selectedBuildingId == null) {
        ...
        javalin.put("/api/admin/staff/{id}", ctx -> {
            ...
            if (user == null || user.roleId() == null || user.roleId() != 1L) {
                ctx.status(HttpStatus.FORBIDDEN);
                ctx.json(ErrorResponse.of("Solo administradores pueden actualizar personal",
                        HttpStatus.FORBIDDEN.getCode()));
                return;
            }
            Long id = Long.parseLong(ctx.pathParam("id"));
        ...
        javalin.delete("/api/admin/staff/{id}", ctx -> {
            ...
            if (user == null || user.roleId() == null || user.roleId() != 1L) {
                ctx.status(HttpStatus.FORBIDDEN);
                ctx.json(ErrorResponse.of("Solo administradores pueden eliminar personal",
                        HttpStatus.FORBIDDEN.getCode()));
                return;
            }
            Long id = Long.parseLong(ctx.pathParam("id"));
